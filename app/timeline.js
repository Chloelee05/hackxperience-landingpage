  ...
  return (
    <div className="mb-10 sm:mb-12">
      <div className="flex items-center gap-3 sm:gap-5 mb-3">
        ...
        <span className="text-xs sm:text-base md:text-xl font-bold tracking-widest text-white whitespace-nowrap font-mono">{title}</span>
        <div className="flex-1 h-px bg-red-700" />
      </div>
      {subtitle && (
...
function ActionButton({ href, children }) {
  const cls = "cursor-pointer border border-gray-500 text-gray-300 text-xs tracking-widest font-mono px-4 sm:px-5 py-2 hover:border-white hover:text-white transition-all duration-200 inline-block"
  if (href && href !== '#') return <a href={href} target="_blank" rel="noopener noreferrer" className={cls}>{children}</a>
  ...
  return (
    <div className="flex gap-4 sm:gap-8">
      ...
      <div className="flex-1 pb-8 sm:pb-10 pt-0.5">
        <Label>{date}</Label>
        <div className="text-white text-base sm:text-xl font-bold tracking-wider font-mono mb-1.5">{title}</div>
        <Meta>{meta}</Meta>
    ...
    <section className="bg-[#1a1a1a] px-4 sm:px-8 md:px-16 pt-12 sm:pt-16 md:pt-20 pb-8 sm:pb-12 font-mono">
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-10 py-6 sm:py-8">
        ...
        <div className="flex-1">
          <div className="text-white text-lg sm:text-2xl font-bold tracking-wider font-mono mb-2 sm:mb-3">BECOME A SPONSOR</div>
